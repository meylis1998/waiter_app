...
class OrderRepositoryImpl extends OrderRepository {
  ...
  Future<void> addOrder(
      int tableId, List<Meal> meals, List<Drink> drinks) async {
    final mealHiveEntities = meals
        .map((meal) => MealHiveEntity(name: meal.name, quantity: meal.quantity))
        .toList();
    final drinkHiveEntities = drinks
        .map((drink) =>
            DrinkHiveEntity(name: drink.name, quantity: drink.quantity))
        .toList();
